...
import { useStockMarkets } from "@/hooks/useStockMarkets";
...
export default function StocksPage() {
  const { stocks, loading, refreshing, error, lastUpdated, refetch } =
    useStockMarkets({ refreshInterval: 60_000, autoRefresh: true });

  const isInitialLoading = loading && !stocks;
  const showCards = stocks && stocks.length > 0;
    ...
    <main className="flex h-screen w-screen flex-col overflow-hidden bg-slate-950 text-slate-100">
      <header className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 px-6 py-4 sm:px-8">
        <div className="flex items-center gap-3 justify-self-start">
          <span
            className="inline-flex h-2.5 w-2.5 rounded-full bg-sky-400 shadow-[0_0_10px_rgba(56,189,248,0.8)]"
            aria-hidden="true"
          />
